...
- The strongest association (r>0.7) was found between IQ and physical activity aggregated time.
- Of particular interest:
- Association between Oros's PSP social dimension and aggregated time was found.

The following is the list of significant results, listing the name of variables, pvalue and Rsquared.

```{r show_corrs_grid, include = TRUE}
## ACTIVITY REGRESSION ANALYSES
res_activity_corr <- regression_significant_main(oactivity_df, joint_nums, varnames_activity, significance_threshold = 0.05, r_min_threshold = 0.07, make_graphics = TRUE, opacity = 0.9, save_graph_to = 'o433activiy_corr.pdf', scatter_cats = FALSE)
## SHOW THE CORRELATIONS GRID
res_activity_corr[['grid']]
```
```{r factor_activity_vars, include=FALSE}
...
The following is the list of significant results, listing the name of variables, pvalue and Rsquared.
```{r show_cats_diffs, include = TRUE}
## SHOW DIFFERENCES P T1 LIST
res_activity_diff[['pr2']]
```

#### RESULTS FROM DIFFERENCE ANALYSES.
...
```{r only_show_cat_diffs, include = TRUE}
## SHOW THE GRID
actdiffgrid
```
...
```{r schools_contingency_tables, include = FALSE}
